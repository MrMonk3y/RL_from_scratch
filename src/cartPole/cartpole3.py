...
def initialize_Q():
    ...
    for state in all_states:
            ...
            for action in range(env.action_space.n):
                    Q[state][action] = np.random.uniform(0, 1) > 0.5
    return Q
...
#        env.render()
        ...
        total_reward += reward
        #punish positions far away from 0
        #total_reward -= abs(observation[0])*0.3
...
def play_many_games(bins, N=10000):
    ...
    envWrapper = wrappers.Monitor(env, 'CartPole3_Movie', force=True)
    ...
    while not done:
        cnt += 1
        act = max_dict(Q[state])[0]
        observation,_ ,done , _ = envWrapper.step(act)
        state = get_state_as_string(assign_bins(observation, bins))
        if done:
            break
