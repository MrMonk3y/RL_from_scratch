...
class QCartPoleSolver():
    def __init__(self, buckets=(1, 1, 6, 12,), n_episodes=10000, n_win_ticks=475.0, min_alpha=0.1, min_epsilon=0.1, gamma=1.0, ada_divisor=25, max_env_steps=None, quiet=False, monitor=False):
    ...
    def run(self):
        ...
        for e in range(self.n_episodes):
            ...
            while not done:
                if e%1000 == 0: self.env.render()
                action = self.choose_action(current_state, epsilon)
                obs, reward, done, _ = self.env.step(action)
                #reward -= 1.7*abs(obs[0])
        ...
        return e
    def test(self):
        self.env = gym.wrappers.Monitor(self.env, 'qcartpole', force=True) # record results for upload
        current_state = self.discretize(self.env.reset())

        alpha = self.get_alpha(1000)
        epsilon = 0
        done = False
        i = 0
